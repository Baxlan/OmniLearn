...
void omnilearn::optimizedUpdate(double& coefToUpdate, LearnableParameterInfos& parameterInfos, bool automaticLearningRate, bool adaptiveLearningRate, bool useMaxDenominator, double learningRate, double momentum,
                                double previousMomentum, double nextMomentum, double cumulativeMomentum, double window, double optimizerBias, size_t iteration, double L1, double L2, double decay, bool avoidZero)
{
    parameterInfos.gradient = parameterInfos.gradient - (L2 * coefToUpdate) - (coefToUpdate > 0 ? L1 : -L1); // regularization
    parameterInfos.previousGrad = (momentum * parameterInfos.previousGrad) + ((1 - momentum) * parameterInfos.gradient); // momentum (FIRST MOMENT OF GRADIENT)
    ...
    double gradientUpdate = ((1 - momentum) * parameterInfos.gradient / (1 - cumulativeMomentum)) + (nextMomentum * parameterInfos.previousGrad / (1 - (cumulativeMomentum * nextMomentum)));
    ...
    parameterInfos.previousGrad2 = (window * parameterInfos.previousGrad2) + ((1 - window) * std::pow(parameterInfos.gradient, 2) / (1-std::pow(window, iteration)));
    ...
    parameterInfos.optimalPreviousGrad2 = (useMaxDenominator ? std::max(parameterInfos.previousGrad2, parameterInfos.optimalPreviousGrad2 * (iteration > 1 ? std::pow((1-momentum) / (1-previousMomentum), 2) : 1)) : parameterInfos.previousGrad2);
    ...
    learningRate = (automaticLearningRate ? std::sqrt((parameterInfos.previousUpdate) + optimizerBias) : learningRate);
    ...
    learningRate /= (adaptiveLearningRate ? std::sqrt((parameterInfos.optimalPreviousGrad2) + optimizerBias) : 1);
    ...
    parameterInfos.previousUpdate = (window * parameterInfos.previousUpdate) + ((1 - window) * std::pow(oldCoef - coefToUpdate, 2) / (1-std::pow(window, iteration)));
